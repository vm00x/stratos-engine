...
export async function POST(req: Request) {
  try {
    ...
    if (!apiKey) return NextResponse.json({ error: "Server misconfigured (Missing Key)" }, { status: 500 });

    const body = await req.json();
    const { url, context, mode } = body; // 'mode' is new

    console.log(`▶️ Processing: ${url ? 'URL Scan' : 'Manual Context'} | Mode: ${mode}`);
    ...
    if (url) {
      ...
      if (!isSafeUrl(url)) return NextResponse.json({ error: "Restricted URL detected" }, { status: 403 });

      try {
        const controller = new AbortController();
        setTimeout(() => controller.abort(), 6000); // 6s timeout
        const response = await fetch(url, { headers: { 'User-Agent': 'StratOS-Enterprise/4.0' }, signal: controller.signal });
        
        if (response.ok) {
          const html = await response.text();
          const $ = cheerio.load(html);
          $('script, style, nav, footer, header, svg').remove();
          sourceMaterial += `WEBSITE LIVE DATA: ${$('body').text().replace(/\s+/g, ' ').slice(0, 3000)} \n\n`;
        }
      } catch (e) {
        sourceMaterial += `URL: ${url} (Connection blocked, using manual context only) \n\n`;
      }
    }
    
    if (context) sourceMaterial += `STRATEGIC CONTEXT: ${context.slice(0, 2000)}`;

    if (!sourceMaterial.trim()) return NextResponse.json({ error: "Input required: URL or Notes" }, { status: 400 });

    // 4. STRATEGIC GENERATION
    // We adjust the prompt based on the selected 'mode'
    const strategyPrompt = `
      ...
      2. "tweet_body": A professional LinkedIn/X post (max 280 chars) aligned with the objective.
